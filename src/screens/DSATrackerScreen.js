...
const DSATrackerScreen = ({ navigation }) => {
  const { theme, questions, communityProgress, toggleQuestionSolved, deleteQuestion, dsaProgress, updateDsaProgress } = useContext(AppContext);
  ...
  // Dynamic list of companies based on loaded DSA questions
  const allCompanies = ['All', ...new Set(dsaQuestions.flatMap(q => q.companyTags || []))];

  const handleIncrement = (topic) => {
    updateDsaProgress(topic, 1);
  };
        ...
        <View style={styles.header}>
          <View style={{ flex: 1 }}>
            ...
            <Text style={{ color: isDark ? colors.textMuted.dark : colors.textMuted.light }}>
              Track your Data Structures and Algorithms prep.
            </Text>
          </View>
          <TouchableOpacity 
        ...
        {Object.entries(categories).map(([categoryName, categoryTopics]) => (
          <View key={categoryName} style={styles.categorySection}>
            ...
            {categoryTopics.map(topic => {
              const topicQuestions = dsaQuestions.filter(q => q.topic === topic);
                    ...
                    <View style={styles.questionsList}>
                      <View style={styles.sectionHeaderRow}>
                        ...
                        <TouchableOpacity 
                          ...
                          onPress={() => navigation.navigate('UploadQuestion', { prefilledType: 'DSA', prefilledTopic: topic })}
                        >
